...
/* Modified by Daniele Denaro 09/2015.
   New functions encript_buff e decrypt_buff are derivated from cbc_encrip and 
   cbc_decripted. New functions replace buffer content and don't need two buffers.
   Also set_key function is changed. Key is generated using a random generator*/
...
void AES::start_key (byte * d, byte nn)
{
  while (nn--)
    *d++ = (byte)random();
}
...
byte AES::encrypt_buff (byte * plain, int n_block)
{
  byte *iv=(byte*)calloc(N_BLOCK,1);
  while (n_block--)
    {
      xor_block (iv, plain) ;
      if (encrypt (iv, iv) != SUCCESS)
        {free(iv);return FAILURE ;}
      copy_n_bytes (plain, iv, N_BLOCK) ;
      plain  += N_BLOCK ;
    }
  free(iv);  
  return SUCCESS ;
...
byte AES::decrypt_buff (byte * cipher,int n_block)
{ 
  byte *iv=(byte*)calloc(N_BLOCK,1); 
  while (n_block--)
    {
      ...
      if (decrypt (cipher, cipher) != SUCCESS)
        {free(iv);return FAILURE ;}
      xor_block (cipher, iv) ;
